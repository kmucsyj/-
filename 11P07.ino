...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, alpha, b_dist_ema, b_dist_raw; // unit: mm
...
// get a distance reading from the USS
  ...
  dist_ema = alpha*dist_raw + (1-alpha)*b_dist_ema;
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min){
    reading = dist_min; // return 0 when out of range.
    digitalWrite(PIN_LED, HIGH);
  }
  else if(reading > dist_max){
    reading = dist_max; // return 0 when out of range.
    digitalWrite(PIN_LED, HIGH);
  }
