...
float dist_min, dist_max, dist_raw, b_dist_raw, duty, duty_per; // unit: mm
...
// get a distance reading from the USS 
  ...
  if(dist_raw == 0) dist_raw = b_dist_raw;
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    duty = map(dist_raw, 100, 200, 255, 0);
    analogWrite(PIN_LED, duty);
  }
  else{
    duty = map(dist_raw, 200, 300, 0, 255);
    analogWrite(PIN_LED, duty);
  }
